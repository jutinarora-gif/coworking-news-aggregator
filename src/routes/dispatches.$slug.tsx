...
function DispatchPage() {
  ...
  return (
    <article className="mx-auto max-w-3xl px-6 py-12">
      <Link to="/dispatches" className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-1"><ArrowLeft className="h-3 w-3" />Back to dispatches</Link>
      <div className="mt-6 text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-2"><span className="acid-dot inline-block h-1.5 w-1.5 rounded-full" />{d.source_name} · {formatDistanceToNow(new Date(d.ingested_at), { addSuffix: true })}</div>
      <h1 className="mt-3 font-display text-4xl md:text-5xl leading-tight">{d.title}</h1>
      <span className="mt-5 block h-[3px] w-14 rounded-full bg-flare" />
      {d.cover_url && (
      ...
      {d.source_url && (
        <a href={d.source_url} target="_blank" rel="noopener noreferrer" className="mt-10 inline-flex items-center gap-2 px-6 py-3 rounded-full bg-flare text-flare-ink font-medium transition-transform hover:-translate-y-0.5">
